...
Page({
  ...
  data: {
    count: 0,
    iShow: false,
    nameList: ["张三", "李四", "王五", "牛二"],
    // 表示名字选择器的选中索引,-1表示默认为空
    index: -1,
    // 时间选择器数据
    time: "12:00",
    // 日期选择器数据
  ...
  myInput(e) {
    ...
    // 小程序通过setData()函数更新页面中的data数据中,参数对象会合并在data对象中
    // this在事件中函数表示的是当前页面对象page
    console.log(this)
    this.setData({
  ...
  initDate() {
    ...
    //  获取默认省份
    var selectProvinces = this.data.provinces[0];
    // 找到省份对应的城市
    var the_cities = cities.filter(item => {
      return item.provinceCode == selectProvinces.code
    })

    // 获取,获取默认城市
    var selectcity = the_cities[0]
    // 找到城市对应的区
